...
def setKeyboardState():
    ...
    for i in range(len(keyboardLayouts)):
        if keyboardStatus[i]:
            layoutString += f"{keyboardLayouts[i]},"
            variantString += f"{keyboardVariants[i]},"
...
def getSystemKeyboardState():
    layoutProcess = subprocess.run(
        "cat /etc/default/keyboard | grep XKBLAYOUT | tr -d '\"'",
        shell=True,
        capture_output=True,
    )
    variantProcess = subprocess.run(
...
def getKeyboardState():
    try:
        ...
        for i in range(len(layouts)):
            for j in range(len(keyboardLayouts)):
                if (
                    layouts[i] == keyboardLayouts[j]
                    and variants[i] == keyboardVariants[j]
                ):
                    keyboardStatus[j] = True
...
def createKeyboardPlugin():
    ...
    subprocess.call(
        [
            ...
            "xfce4-panel",
            "-p",
            f"/plugins/{keyboardPlugin}/display-name",
            "-s",
            "1",
            "--type",
            "int",
    ...
    subprocess.call(
        [
            ...
            "xfce4-panel",
            "-p",
            f"/plugins/{keyboardPlugin}/display-type",
            "-s",
            "2",
            "--type",
            "int",
    ...
    subprocess.call(
        [
            ...
            "xfce4-panel",
            "-p",
            f"/plugins/{keyboardPlugin}/group-policy",
            "-s",
            "0",
            "--type",
            "int",
...
def getKeyboardPlugin():
    ...
    pluginList = (
        subprocess.check_output(["xfconf-query", "-c", "xfce4-panel", "-l", "-v"])
        .decode("utf-8")
        .splitlines()
    )
